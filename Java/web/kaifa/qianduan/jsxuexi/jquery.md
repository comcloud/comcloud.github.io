...
<script>
	//这个时候因为引入了两个库文件，所以在两次导入的时候第二个会将第一个的赋值$给覆盖掉
   	//如果依旧想要使用第一个库文件，那么可以使用jQuery来代替$，或者使用下面方法的返回值
    var _$ = $.noConflict();//这个方式是释放掉下面覆盖库文件对$的控制权，并且将上一个$返回
	
	//如果写入了很多代码后，使用的都是$符号，引入了新的库文件而不想一个个更换$为_$：
	(function($){
        //此时这里的参数$就是传入的_$，然后将原定的代码放入到这个自执行函数中
    }(_$));
